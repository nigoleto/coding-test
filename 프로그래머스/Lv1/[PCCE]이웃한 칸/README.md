...
![Example1](https://grepp-programmers.s3.ap-northeast-2.amazonaws.com/files/production/cb8c0433-a307-4184-b224-6185c87dfc07/9-1.jpg)

#### 입출력 예 #2

주어진 보드를 나타내면 아래 그림과 같고, `board[0][1]`의 인접한 칸들은 화살표가 가리키는 칸들입니다. 따라서 `board[0][1]` 주변의 "green"칸은 총 1개입니다.

![Example2](https://grepp-programmers.s3.ap-northeast-2.amazonaws.com/files/production/a9fdec91-df64-4240-82b3-c63d9555e689/9-2.jpg)

---

### 참고 사항

- C++ 응시자는 리스트를 배열과 동일한 의미로 해석하여 풀이에 참고하세요.  
  - 예: 번호가 담긴 정수 **리스트** `numbers`가 주어집니다. → 번호가 담긴 정수 **배열** `numbers`가 주어집니다.  
- Java 응시자는 리스트를 배열, 함수를 메서드와 동일한 의미로 해석하여 풀이에 참고하세요.  
  - 예: solution **함수**가 올바르게 작동하도록 한 줄을 수정해 주세요. → solution **메서드**가 올바르게 작동하도록 한 줄을 수정해 주세요.
